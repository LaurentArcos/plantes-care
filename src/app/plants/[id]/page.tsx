...
async function getPlant(id: string): Promise<UIPlant | null> {
  ...
  const res = await fetch(
    `${base}/api/plants?id=${encodeURIComponent(id)}&limit=1`,
    { cache: "no-store" }
  );
...
export default async function PlantDetail({
  ...
  const p = await getPlant(id);            // ✅ on passe l'id

  if (!p) {
    return (
      <div className="p-4 bg-white border rounded-2xl">
        <div className="text-sm text-neutral-600">Introuvable.</div>
        <Link href="/plants" className="inline-block mt-3 text-sm underline text-green-700">
          ← Retour aux plantes
        </Link>
      </div>
    );
    ...
    <div className="grid gap-6">
      <div className="overflow-hidden border rounded-2xl bg-white">
        ...
        <div className="p-4">
          ...
          <div className="mt-2 text-sm text-neutral-700">Lieu : {p.location_name ?? "—"}</div>
